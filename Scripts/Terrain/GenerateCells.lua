...
function generateOverworldCelldata(xMin, xMax, yMin, yMax, seed, data, padding)
    ...
    for y = yMin + 1, yMax - 1 do
        for x = xMin + 1, xMax - 1 do
            local gradC =
                ...
                math.min(
                    math.max(
                        math.abs(x - xCenter) / (xHalfSize - padding),
                        math.abs(y - yCenter) / (yHalfSize - padding)
                    ),
                    1
                )
            ...
            local island =
                sm.util.clamp(sm.noise.perlinNoise2d(x / 6, y / 6, seed + 6437) * 2 + 0.5, 0, 1) +
                sm.util.clamp(gradC * 2 - 1, -1, 1)
    ...
    pois[#pois + 1] = {
        x = -27,
        y = -26,
        type = POI_MECHANICSTATION_MEDIUM,
        size = 2,
        road = false,
        flat = true,
        terrainType = TYPE_FOREST,
    ...
    pois[#pois + 1] = {
        x = -17,
        y = -23,
        type = POI_PACKINGSTATIONVEG_MEDIUM,
        size = 2,
        road = false,
        flat = true,
        cellDebug = nil,
    ...
    pois[#pois + 1] = {
        x = 0,
        y = -21,
        type = POI_PACKINGSTATIONFRUIT_MEDIUM,
        size = 2,
        road = false,
        flat = true,
        cellDebug = nil,
    ...
    pois[#pois + 1] = {
        x = 24,
        y = 0,
        type = POI_CAPSULESCRAPYARD_MEDIUM,
        size = 2,
        road = false,
        flat = true,
        cellDebug = nil,
    ...
    pois[#pois + 1] = {
        x = 24,
        y = -20,
        type = POI_LABYRINTH_MEDIUM,
        size = 2,
        road = false,
        flat = true,
        cellDebug = nil,
        terrainType = TYPE_FIELD,
        edges = {}
    }
    ...
    for _, spot in ipairs(largePoiSpots) do
        ...
        local poi = {
            x = spot.x,
             -- + sm.noise.intNoise2d( spot.x, spot.y, seed + 3831 ) % 9 - 4,
            y = spot.y,
             -- + sm.noise.intNoise2d( spot.x, spot.y, seed + 4041 ) % 9 - 4,
            type = type,
            size = 4,
        ...
        for j = i + 1, #roadPois do
            if isNeighbor(roadPois, i, j) then
                ...
                local distanceMultiplier =
                    math.max(
                    DistanceCostMultipliers[g_cornerTemp.type[a.y][a.x]],
                    DistanceCostMultipliers[g_cornerTemp.type[b.y][b.x]]
                )
    ...
    local convertToRoadPoi = function(poi, other)
        ...
        if
            poi.rotation == nil and
                (poi.type == POI_MECHANICSTATION_MEDIUM or poi.type == POI_PACKINGSTATIONVEG_MEDIUM or
                    poi.type == POI_PACKINGSTATIONFRUIT_MEDIUM)
         then
        ...
        if
            poi.rotation == nil and
                (poi.type == POI_WAREHOUSE2_LARGE or poi.type == POI_WAREHOUSE3_LARGE or
                    poi.type == POI_WAREHOUSE4_LARGE)
         then
    ...
    for y = yMin, yMax + 1 do
        ...
        for x = xMin + 1, xMax + 1 do
            if g_cornerTemp.lakeAdjacent[y][x] then
                ...
                g_cornerTemp.hillyness[y][x] =
                    math.min(g_cornerTemp.hillyness[y][x - 1] + 0.2, g_cornerTemp.hillyness[y][x])
            end
        end
        -- Sweep east to west
    ...
    for x = xMin, xMax + 1 do
        ...
        for y = yMin + 1, yMax + 1 do
            if g_cornerTemp.lakeAdjacent[y][x] then
                ...
                g_cornerTemp.hillyness[y][x] =
                    math.min(g_cornerTemp.hillyness[y - 1][x] + 0.2, g_cornerTemp.hillyness[y][x])
            end
        end
    ...
    forEveryCorner(
        function(x, y)
            if g_cornerTemp.lakeAdjacent[y][x] then
                ...
                elevation =
                    elevation +
                    sm.noise.perlinNoise2d(x / 16, y / 16, seed + 7907) *
                        clamp((g_cornerTemp.gradC[y][x] * 3 - 1), 0, 1)
    ...
    forEveryCell(
        function(x, y)
            ...
            if
                g_cellData.flags[y][x] == 0 and getCornerType(x + 1, y) == type and getCornerType(x, y + 1) == type and
                    getCornerType(x + 1, y + 1) == type
             then
        ...
        for j = 0, 42 do
            ...
            while k < 2 do
                ...
                while l < 2 do
                    ...
                    if
                        g_cellData.flags[y][x] == 0 and getCornerType(x + 1, y) == terrainType and
                            getCornerType(x, y + 1) == terrainType and
                            getCornerType(x + 1, y + 1) == terrainType
                     then
                        -- Find poi box distance
                        local minBoxDst = math.huge
                        for _, poi in pairs(pois) do
